...
void node::PublishPoseAsTransform(const Sophus::SE3f& Twc, double timestamp) {
  ...
  Eigen::Affine3d map_to_camera_affine(Eigen::Translation3d(Twc.translation().cast<double>()) * Twc.unit_quaternion().matrix().cast<double>());
  ...
  try {
    if(!mbIMU)
      map_to_camera_affine = (map_to_camera_affine * rot_ros_to_cv_map_frame_.inverse());
  
    auto optical_link_to_camera_link = tf_->lookupTransform(left_cam_frame_id_, camera_link_id_, ros::Time(0));
    Eigen::Affine3d optical_link_to_camera_link_affine = tf2::transformToEigen(optical_link_to_camera_link.transform);
    ...
    map_to_camera_link_msg.header.frame_id = world_frame_id_;
    map_to_camera_link_msg.child_frame_id = camera_link_id_;
  ...
  // tfMsg.header.frame_id = world_frame_id_;  // world OR map
  // tfMsg.child_frame_id = left_cam_frame_id_;
