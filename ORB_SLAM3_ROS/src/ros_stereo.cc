...
void VIO::GrabStereo(const sensor_msgs::ImageConstPtr &msgLeft,
    ...
    if (mbRectify) {
        ...
        Tcw = mORB_SLAM3->TrackStereo(imLeft, imRight, cv_ptrLeft->header.stamp.toSec());
    } else {
        Tcw = mORB_SLAM3->TrackStereo(cv_ptrLeft->image,
                                      cv_ptrRight->image,
                                      cv_ptrLeft->header.stamp.toSec());
    }
    ORB_SLAM3::KeyFrame *pRefKF = mORB_SLAM3->GetTracker()->mCurrentFrame.mpReferenceKF;
    Update(Tcw, cv_ptrLeft->header.stamp.toSec());
