  ...
  vdStereoMatch_ms.push_back(mCurrentFrame.mTimeStereoMatch);
    ...
    void Tracking::PreintegrateIMU() {
        ...
        if (mlQueueImuData.empty()) {
            std::cout << "Not IMU data in mlQueueImuData!!" << std::endl;

            // mCurrentFrame.setIntegrated();
            return;
        }
        // std::cout << "Queue size: " << mlQueueImuData.size() << std::endl;
        while (true) {
            bool bSleep = false;
            {
                unique_lock <mutex> lock(mMutexImuQueue);
        ...
        // std::cout << "Vector size: " << mvImuFromLastFrame.size() << std::endl;
...
//      } else
    ...
    vdLMTrack_ms.push_back(timeLMTrack);
            ...
            if (mCurrentFrame.isSet()) {
                ...
                mpMapDrawer->setmTimestamp(mCurrentFrame.mTimeStamp);
            }
    ...
    void Tracking::CreateInitialMapMonocular() {
        ...
        mpMapDrawer->setmTimestamp(pKFcur->mTimeStamp);
    ...
    bool Tracking::TrackWithMotionModel() {
        ...
        if (mpAtlas->isImuInitialized() &&
            ...
            mCurrentFrame.imuIsPreintegrated() &&
            !mbEmptyImu) {
            // Predict state with IMU if it is initialized and it doesnt need reset
            // Vuong: after VIBA2 let use constant velocity for more stable and less computation
            ...
            if (mbEmptyImu) {
                cout << "Predict state only using VO, because IMU measurements vector is empty\n";
                mbEmptyImu = false;
            }
            mCurrentFrame.SetPose(mVelocity * mLastFrame.GetPose());
        }
    ...
    bool Tracking::TrackLocalMap() {
        ...
        int inliers;
        // in case of imu is not init or is not preintegrated
        if (!mpAtlas->isImuInitialized() || !mCurrentFrame.imuIsPreintegrated() ||
            mCurrentFrame.mpImuPreintegratedFrame == nullptr || mCurrentFrame.mpcpi == nullptr ||
            mCurrentFrame.mpImuPreintegrated == nullptr)
            Optimizer::PoseOptimization(&mCurrentFrame);
        else {
            if (mCurrentFrame.mnId <= mnLastRelocFrameId + mnFramesToResetIMU) {
    ...
    void Tracking::UpdateLocalPoints() {
        ...
        for (vector<KeyFrame *>::const_reverse_iterator
            ...
            for (vector<MapPoint *>::const_iterator itMP = vpMPs.begin(),
                ...
                if (!pMP->isBad()) { // TODO: What are the criteria?
                    count_pts++;
                    mvpLocalMapPoints.push_back(pMP); // Here is an important place when the points are given
